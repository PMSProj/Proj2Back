...
public class FriendDaoImpl implements FriendDao {
	...
	public List<User> suggestedUsers(String email) {
		...
		String queryString = "select * from user_info where email in(select email from user_info where email!=? minus(select toId_email from friend where fromId_email=? union select fromId_email from friend where toId_email=?))";
